...
- feat(memory-pgvector): **ADR-021 Phase 2b — backend tenant-scoped query path.** `PgPoolLike` gains an optional `withRlsScope(tenantId, fn)` (external `createPgPool` implements it via `BEGIN` → `SET LOCAL ROLE <appRole>` → `set_config('app.tenant', …)`); ingest/search/delete accept an optional `tenantId` and route through it **only when** a tenant is supplied and the pool supports it, stamping `tenant_id` on writes and letting RLS isolate reads/deletes. Embedded PGlite and no-tenant calls are byte-identical to before. The backend takes **no** dependency on `@onenomad/przm-access` — it handles a `tenantId` string; the server owns the Principal (Phase 3). Proven by a backend-level isolation test.
- feat(server): `createPgVectorClient` accepts `appRole` (external + RLS only), forwarded to the scoped pool.
- feat(server): **ADR-021 Phase 3 — token → Principal → tenant-scoped queries.** The MCP HTTP transport verifies a przm-access EdDSA scoped token (new `PRZM_CORTEX_ACCESS_PUBLIC_JWK` / `_ISSUER` / `_AUDIENCE`); a valid token both authenticates and stamps a `Principal` on the session, and the memory client injects that tenant into `ingest`/`search` so RLS isolates them. Verify is cortex-side (jose, public-key-only) so the contract package stays thin; `@onenomad/przm-access` + `jose` added as server deps. New config `memory.pgvector.enableRls` + `appRole` turn external multi-tenant mode on. **Fully opt-in and gated** — with the access key unset and `enableRls` off, behavior is unchanged (single-tenant; stdio stays local). The token's tenant is the authoritative isolation key; the workspace slug stays a label.
- docs: ADR-021 (przm-access → cortex integration) accepted; the multi-tenant RLS substrate lands across five phases. Phase 0 (RLS hardening + publishable contract) shipped in przm-access; Phases 1 (schema + policies), 2a (contract adoption + integration proof), and 2b (backend scoped-query path) here. Phase 3 next (token→Principal on the transport). R4 resolved: `@onenomad/przm-access` is published Apache-2.0 (v0.1.1) on public npm.
