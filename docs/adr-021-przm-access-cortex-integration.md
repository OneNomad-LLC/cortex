...
- **R4 — RESOLVED: contract package ships as a private npm package.** `@onenomad/przm-access` publishes to npm under the `@onenomad` scope with `publishConfig.access: "restricted"` — installable only with npm auth, never public. cortex consumes it as an ordinary versioned dependency (same `@onenomad/*` scope as the public `przm-cortex-*` packages). The package's `package.json` flips from `"private": true` to publishable; cortex CI/install needs an npm token with read access to the scope.
