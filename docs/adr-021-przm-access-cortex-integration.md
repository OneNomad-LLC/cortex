...
- **Phase 2 — scoped query path (cortex).** Grow the external `PgPoolLike` to a `withRlsSession`-capable shape; route external-mode reads/writes through it with the `Principal`. Keep `$eqOrNull` as the in-query belt.
  ...
  - **Phase 2b — DONE.** The backend scoped-query path, **without** a przm-access runtime dep (keeps `memory-pgvector` self-contained + Apache-2.0). Design:
    - `PgPoolLike` gains an optional `withRlsScope(tenantId, fn)`; the external `createPgPool` implements it (`BEGIN` → `SET LOCAL ROLE <appRole>` → `set_config('app.tenant', tenantId, true)` → run `fn` → COMMIT). Embedded PGlite omits it.
    - `MemoryIngestInput` / `MemorySearchArgs` / `MemoryDeleteArgs` gain an optional `tenantId: string`. The backend routes ingest/search/delete through `withRlsScope(tenantId, …)` **only when** a `tenantId` is supplied AND the pool implements `withRlsScope`; otherwise the existing plain `pool.query` path runs (embedded / no tenant = byte-identical behavior). Ingest stamps `tenant_id`; reads/deletes need no SQL change — RLS does the filtering once `app.tenant` is set.
    - The backend deals only with a `tenantId` **string** — it never imports `@onenomad/przm-access`. The **server** owns the contract dependency and (Phase 3) derives `tenantId` from a verified `Principal`, passing it in the op args + supplying `appRole`/`enableRls` to `createPgVectorClient`.
    ...
    - **Not yet wired:** admin/maintenance ops (`wipeAll`, `exportAll`) are not tenant-scoped — they have no per-request tenant context and stay on the unscoped path; revisit when an admin Principal exists.
  - **Phase 3 — DONE.** The `Principal` source, end to end:
    - Token verify lives in **cortex** (`server/src/access/verify-token.ts`, `createAccessVerifier` — jose, EdDSA, public-key-only → `Principal`), **not** the contract package (it stays near-zero-dep). The server adds `@onenomad/przm-access` (Apache-2.0) as a runtime dep for the `Principal` type + `jose`.
    - The MCP **HTTP** transport builds a verifier from env (`PRZM_CORTEX_ACCESS_PUBLIC_JWK` / `_ISSUER` / `_AUDIENCE`); a valid przm-access bearer both **satisfies auth** and yields a `Principal`, stamped on the session (`setSessionPrincipal`, mirroring the scope-token/`setSessionToolAllowList` path). Re-derived per request; cleared when absent (no cross-request leak). Entirely opt-in — unset key ⇒ inert, existing auth unchanged. **stdio stays single-tenant/local** (no header channel).
    - `tenantId` is injected **centrally** in the EngramClient wrapper (`clients/pgvector.ts` reads `getCurrentTenantId()` and passes it into `backend.ingest`/`search`) — **no per-tool changes**. Absent principal ⇒ no `tenantId` ⇒ unscoped (embedded/default behavior).
    - Config wiring: `memory.pgvector.enableRls` + `appRole` (config schema → `clients/memory.ts` → external client). `workspace → tenant`: the token's `tenantId` is the authoritative RLS boundary; the workspace slug remains a human label and its in-query filter stays as a belt (defense in depth).
    - Tests: `access-verify-token.test.ts` (verify round-trip + tamper/expiry/aud/iss/malformed rejection), `session-principal.test.ts` (principal → `getCurrentTenantId` threading + no-leak on clear).
    - **Follow-on:** a full e2e (token → HTTP transport → RLS-scoped query against real Postgres) — both ends are unit-proven; the live wiring is covered by typecheck. Worth a CI e2e before a production multi-tenant claim.
